...
class AddressesTable extends Table
{
    ...
    {
        $validator
            ->requirePresence('postal_code', 'create', '(postal_code) | "CEP" é obrigatório')
            ->notEmptyString('postal_code', '(postal_code) | "CEP" não pode ser vazio.')
            ->regex('postal_code', '/^\d{8}$/', '(postal_code) | "CEP" deve ter exatamente 8 dígitos');

        $validator
            ->notEmptyString('sublocality', '(sublocality) | "Bairro" não pode ser vazio.');

        $validator
            ->notEmptyString('street', '(street) | "Logradouro" não pode ser vazio.');

        $validator
            ->requirePresence('street_number', 'create', '(street_number) | "Número" é obrigatório')
            ->notEmptyString('street_number', '(street_number) | "Número" não pode ser vazio');
